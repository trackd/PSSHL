...
function DeserializeResponse {
    ...
    If ([string]::IsNullOrEmpty($ReturnType) -and $ContentTypes.Count -eq 0) {
        # void response
        return $Response
    } Elseif ($ReturnType -match '\[\]$') {
        # array
        return ConvertFrom-Json $Response
    } Elseif (@('String', 'Boolean', 'System.DateTime') -contains $ReturnType) {
        # string, boolean ,datetime
        return $Response
    } Else {
        # others (e.g. model, file)
        if ($ContentTypes) {
            $ContentType = $null
            ...
            # no content type in response header, returning raw response
            return $Response
        }
    }
}
